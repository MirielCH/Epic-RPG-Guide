...
async def horses(prefix):
    ...
    tier =  f'{emojis.bp} Tiers range from I to IX (1 to 9)\n'\
            ...
            f'{emojis.bp} Mainly increased by breeding with other horses\n'\
            f'{emojis.bp} Very small chance of increasing in horse races\n'\
            f'{emojis.bp} **Always** breed with a horse of the same tier'
            
    level = f'{emojis.bp} Levels range from 1 to (tier * 10)\n'\
            f'{emojis.bp} Leveling up increases the horse type bonus (see the [Wiki page](https://epic-rpg.fandom.com/wiki/Horse#Horse_Types_and_Boosts))\n'\
    ...
    type =  f'{emojis.bp} There are 5 different types\n'\
...
async def horsetypes(prefix):
    ...
    special =   f'{emojis.bp} Unlocks the epic quest which gives more coins and XP than the regular quest\n'\
                f'{emojis.bp} The higher the horse level, the more coin and XP you get\n'\
    ...
    embed.add_field(name=f'ADDITIONAL GUIDES', value=f'{emojis.bp} `{prefix}h` : Horses overview\n{emojis.bp} `{prefix}htier` : Details about horse tiers', inline=False)
