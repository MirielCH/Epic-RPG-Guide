...
class CraftingCog(commands.Cog):
    ...
    async def craft(
        ...
        amount: Option(int, 'The amount of items you want to see the materials for. Uses 1 if empty.',
                       min_value=1, max_value=100_000_000_000, default=1),
    ) -> None:
    ...
    async def inventory_calculator(
        ...
        material: Option(str, 'The material convert to', choices=INVCALC_ITEMS),
    ) -> None:
        """Dropchance calculator"""
        bot_message_task = asyncio.ensure_future(functions.wait_for_inventory_message(self.bot, ctx))
        try:
            bot_message = await functions.wait_for_bot_or_abort(ctx, bot_message_task, 'rpg i')
        except asyncio.TimeoutError:
            await ctx.respond(strings.MSG_BOT_MESSAGE_NOT_FOUND.format(user=ctx.author.name, information='inventory'))
            return
        if bot_message is None: return
        ...
        response = (
            f'**{ctx.author.name}**, your inventory equals **{amount:,}** {item.emoji} {item.name}.\n'
