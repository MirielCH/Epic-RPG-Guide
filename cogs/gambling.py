...
class GamblingCog(commands.Cog):
    ...
    async def gambling_blackjack(
        self,
        ctx: discord.ApplicationContext,
        topic: Option(str, strings.ARGUMENT_TOPIC_DESCRIPTION, choices=TOPICS),
    ) -> None:
        """Gambling guides"""
        topics_functions = {
...
async def embed_multidice() -> discord.Embed:
    ...
    note = (
        ...
        f'{emojis.BP} The amount you can bet is limited by your coin cap (see `/coincap`)'
    )
    embed = discord.Embed(
        color = settings.EMBED_COLOR,
        title = 'MULTIDICE',
