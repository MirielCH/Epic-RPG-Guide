...
async def find_message(channel_id: int, regex: Union[str, re.Pattern] = None,
                      user_id: Optional[int] = None, user_name: Optional[str] = None) -> discord.Message:
    ...
    user_id: User ID the message author has to match.
    user_name: User name the message author has to match. If user is also defined, this is ignored.
    If both user_id and user_name are None, this function returns the first message that matches the regex.
    ...
    attempts = 1
    while attempts <= 2:
        if regex is None and user_id is None and user_name is None:
        ...
        for message in channel_messages:
            if user_id is not None and message.author.id != user_id: continue
            if (user_name is not None
                and functions.format_string(user_name) != functions.format_string(message.author.name)):
                continue
            if regex is None:
                _MESSAGE_CACHE[channel_id] = []
                return message
            else:
